...
static inline u32 le32_to_h(u8 *data)
{
	return data[0] | (data[1] << 8) | (data[2] << 16) | (data[3] << 24);
}

static inline u32 le24_to_h(u8 *data)
{
...
static u32 mmc_read_blocks(struct mmc *mmc, void *dst, lbaint_t start,
				   lbaint_t blkcnt)
{
	struct mmc_cmd cmd;
	struct mmc_data data;
...
static enum cb_err sd_send_op_cond(struct mmc *mmc)
{
	int timeout = 1000;
	enum cb_err err;
	struct mmc_cmd cmd;
...
static enum cb_err mmc_send_ext_csd(struct mmc *mmc, u8 *ext_csd)
{
	struct mmc_cmd cmd;
	struct mmc_data data;
...
static enum cb_err mmc_switch(struct mmc *mmc, u8 set, u8 index, u8 value)
{
	struct mmc_cmd cmd;
	int timeout = 1000;
	int ret;
...
static enum cb_err mmc_change_freq(struct mmc *mmc)
{
	...
	if ((err = mmc_send_ext_csd(mmc, ext_csd)) != CB_SUCCESS)
		return err;
...
static enum cb_err mmc_enable_spi_crc(struct mmc *mmc)
{
	struct mmc_cmd cmd;
...
static enum cb_err mmc_read_id(struct mmc *mmc, void *dest)
{
	struct mmc_cmd cmd;
	enum cb_err err;
...
static enum cb_err mmc_startup(struct mmc *mmc)
{
	...
	if (mmc_host_is_spi(mmc))
		if ((err = mmc_enable_spi_crc(mmc)) != CB_SUCCESS)
			return err;

	/* Put the Card in Identify Mode */
	if ((err = mmc_read_id(mmc, mmc->cid)) != CB_SUCCESS)
		return err;
	...
	if (!card_is_sd(mmc) && (mmc->version >= MMC_VERSION_4)) {
		...
		if (!err && (ext_csd[EXT_CSD_REV] >= 2)) {
			...
			capacity = le32_to_h(&ext_csd[EXT_CSD_SEC_CNT]);
		...
		for (i = 0; i < 4; i++) {
			...
			mmc->capacity_gp[i] = le24_to_h(&ext_csd[idx]);
			mmc->capacity_gp[i] *=
			    ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE];
	...
	if ((err = mmc_set_capacity(mmc, mmc->part_num)) != CB_SUCCESS)
		return err;
...
/**
 * \brief Start device initialization
 *
...
enum cb_err mmc_init(struct mmc *mmc)
{
	...
	if ((err == CB_SUCCESS) || (err == CB_MMC_IN_PROGRESS))
...
/**
 * \brief Set preinit flag of mmc device.
 *
